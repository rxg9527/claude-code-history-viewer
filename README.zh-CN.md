...
- 无头 server 的 Homebrew 分发：`brew install rxg9527/tap/cchv-server`
...
> Fork 说明：这个 fork 提供 server 的 Homebrew formula，
> 但不提供桌面端 Homebrew cask。桌面应用请直接使用 GitHub Releases，或从源码构建。
...
这个 fork 不提供桌面端 Homebrew cask。
请使用 GitHub Releases，或直接从源码构建桌面应用。

无头 server 可通过 Homebrew 安装：
