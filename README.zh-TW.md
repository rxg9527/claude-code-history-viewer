...
- 無頭 server 的 Homebrew 發佈：`brew install rxg9527/tap/cchv-server`
...
> 這個 fork 提供無頭 server 的 Homebrew formula，
> 但不提供桌面端 Homebrew cask。桌面應用請使用 GitHub Releases 或從原始碼建置。
...
這個 fork 不提供桌面端 Homebrew cask。
桌面應用請使用 GitHub Releases，或直接從原始碼建置。

無頭 server 可透過 Homebrew 安裝：
